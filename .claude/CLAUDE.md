...
**As of 26 August 2026 it is a personal site pointed at a job search**: projects and personality, not an offer. Tom is looking for full-time work (junior for general software, any non-senior level for AI) and every CV links here, so the reader is a hiring manager. The paid work lives on the commercial site (thomasjbutler.me), and the hero says so on purpose: everything here was built because he wanted to. Self-assessed skill bars and unverifiable stat grids are still gone and still must not come back; the "Buy me a coffee" link is back (footer and /contact) because on a personal site it is just a link.

### Shelved positioning (restore when the job search is over)

From July to August 2026 this site was a **shop window for Local & Private AI** ("AI you own, not AI you rent"): a `/services` route, three priced offers, engagement terms, a retainer, an FAQ, a newsletter strip, a ProfessionalService JSON-LD block and a price-drift test. Tom expects to bring that back in late 2026 or 2027. It is **preserved verbatim in the git tag `v-local-ai-2026-08`** (on `e71241f`), and the commit that removed it is the one to revert: `git log --grep="Shelve the Local AI offer"`. A revert will conflict in the files later commits also touched (`content.ts`, `routes.mjs`, `index.html`, `llms.txt`); the tag is the byte-for-byte reference for resolving them. `/services` redirects to `/about` in the meantime so old inbound links land on the person: a static `services.html` (same shape as `blog.html`, a Vite input, copied to `services/index.html` by the prerender) for the instant server-side case, plus a `<Navigate>` in `App.tsx` for in-app navigation. `/skills` has only the `<Navigate>`, which on GitHub Pages means the 404 shell, a hydration mismatch and six to twelve seconds before About appears; that is pre-existing and shared by every legacy redirect.

**Truth constraints that outlive the positioning** (from Tom's CV rules; do not breach in any copy):
- Location is **Leeds, Yorkshire**, everywhere. Not York, not Liverpool, not Wetherby.
- Level 4 Software Developer Apprenticeship: "**studied 2023-24**". Never completed, never achieved.
- Tom has **never delivered local-AI work for a business**, paid or unpaid. Local models are personal use, over a year. Never imply clients.
- Never "**Founder**" as a title. Describe the work.
- **No explicit ADHD diagnosis** anywhere. "Built from lived experience" is the phrasing.
- Sanctuary is on-device only, iOS 27. Round statistics only (~23,000 tests, ~88%), never exact figures.
- Voice: contractions, no em dashes, UK spelling, "properly" not "genuinely", none of the CV banned phrases ("passionate about", "leverage", "cutting-edge", "proven track record", "delve").
...
- **The prices, revision terms and retainer are in the tag, not the tree.** `PRICING`, `ENGAGEMENT_TERMS`, `RETAINER` and their sourcing comment (benchmarked against 2026 UK market data) live in `src/lib/content.ts` at `v-local-ai-2026-08`. Do not retype figures from memory; check out the tag.
...
- **The case study's `provenance` field is not padding.** The ISQ Agent build ran on hosted models (Claude, Voyage, Pinecone) while the write-up argues for running the model where you control it, and the page renders those badges. It was a technical assessment for RiverAI, not client work, and says so. Stating both plainly is the only thing that keeps the page honest; don't quietly delete it.
...
- **The Updates timeline is `src/lib/timeline.ts` and nothing else.** A dead duplicate (`src/utils/timelineData.ts`, zero importers) used to sit beside it and silently kept every old string; it was deleted in August 2026. If one reappears, delete it again.
- **`scripts/routes.mjs` is a runtime import** (`Layout.tsx` uses `titleForPath`), not just a build script. Removing a route means: the entry there, the `<Route>` in `App.tsx`, `public/sitemap.xml` (hand-maintained), and any `module:` that no longer resolves in the Vite manifest (a hard build error). Leaving the entry while removing the route prerenders the 404 into that file and trips the 600-character floor.
